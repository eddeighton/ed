        ...
        m_pParent( _pParent )
{
    for( PtrVector::const_iterator
        i = n.m_children.begin(),
        iEnd = n.m_children.end();
        i!=iEnd; ++i )
    {
...
EdNode::EdNode( const Node& n, EdNode* _pParent )
    :   m_statement( n.statement ),
        m_pParent( _pParent )
{
    for( Node::Vector::const_iterator
        i = n.children.begin(),
        iEnd = n.children.end();
        i!=iEnd; ++i )
    {
...
{
    bool bEqual = false;
    if( left.getStatement() == right.getStatement() &&
        left.getChildren().size() == right.getChildren().size() )
    {
        EdNode::PtrVector::const_iterator i = left.getChildren().begin(),
            iEnd = left.getChildren().end(),
            j = right.getChildren().begin();
        for( ; i!=iEnd && **i==**j; ++i, ++j );
            ...
            if( uiIndex != TagList::NO_INDEX )
            {
                ...
                if( getStatement().shorthand )
                {
                        ...
                        getStatement().shorthand.get().get( uiIndex ) )
                    {
                        ...
                        overrideAndExtend( pNode->m_statement.shorthand, newShorthandOpt ); //now override and extend the original child shorthand with it
...
boost::filesystem::path BasicFileSystem::load( const std::string& strRelativePath, std::string& strContents )
{
    const boost::filesystem::path filePath = 
        boost::filesystem::edsCannonicalise(
            boost::filesystem::absolute( strRelativePath ) );
    ...
    :   m_filesystem( filesystem )
{
    ...
    for( Node::Vector::const_iterator i = m_originals.begin(), iEnd = m_originals.end();i!=iEnd; ++i )
        m_children.push_back( new EdNode( *i, this ) );
...
boost::optional< const EdNode& > File::lookupType( const EdNode* pNode,
                    const TypeList::const_iterator iter,
                    const TypeList::const_iterator iterEnd )
{
    ...
    else
    {
        for( EdNode::PtrVector::const_iterator
            i = pNode->m_children.begin(),
            iEnd = pNode->m_children.end(); i!=iEnd; ++i )
        {
            if( !(*i)->getStatement().declarator.identifier &&
                !(*i)->getStatement().declarator.typeList.empty() )
            {
        ...
        if( !result )
        {
            ...
            PtrMap::const_iterator iFind = m_inheritedFiles.find( pNode );
...
void File::inheritStatement( EdNode* pNode )
{
    if( pNode->m_statement.declarator.identifier )
    {
        ...
        else
        {
            ...
            VERIFY_RTE_MSG( !pNode->m_statement.getFileRefs( files ), "Node cannot have both type and inheritance directive: " << pNode->m_statement );
...
struct LocateRefFromRootVisitor : boost::static_visitor< boost::optional< const EdNode& > >
{
    ...
    boost::optional< const EdNode& > operator()( const Identifier& identifier ) const
    {
        ...
        for( EdNode::PtrVector::const_iterator
            i = nodes.begin(), iEnd = nodes.end(); i!=iEnd; ++i )
        {
            if( (*i)->getStatement().declarator.identifier &&
    ...
    boost::optional< const EdNode& > operator()( const RefActionType& type ) const
    {
        ...
        return boost::optional< const EdNode& >();
    }
...
boost::optional< const EdNode& > getRefNode( const EdNode::PtrVector& nodes,
        Reference::const_iterator i, Reference::const_iterator iEnd )
{
    boost::optional< const EdNode& > result;
    if( i != iEnd )
    {
...
{
    const boost::filesystem::path p = boost::filesystem::edsCannonicalise(
    ...
    for( Node::Vector::const_iterator i = node.children.begin(),
